/*
该函数计算特定范围内所有证书的平方和。程序限制了范围的上限是10000000, 下限是-10000000
*/
#include <stdio.h>
#include <stdbool.h>
//验证输入是一个整数
...
int main(void) {
	...
	printf("This program computes the sum of the squares of integers in a range.\nThe lower bound shouldnot be less than -10000000 and \n the upper bound should not be more than +10000000.\n"
		" Enter the limits (enter 0 for both limits to quit): \b Lower limit: \n");
	start = get_long();
	printf("upper limit: \n");
	stop = get_long();
	while (start != 0 || stop != 0)
	{
		...
		} else
		{
			...
			printf("The sum of the squatres of the integers from %ld to %ld is %g\n", start, stop, answer);		
		}
